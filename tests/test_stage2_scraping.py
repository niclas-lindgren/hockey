...
class TestLLMFallback:
    """Tests for the LLM fallback path in stage2 run()."""

    def _blocked_llm_source_result(self, name: str, url: str = "https://llm.example.com") -> dict:
        """Build a source_result dict that simulates a source marked for LLM fallback."""
        return {
            "name": name,
            "url": url,
            "type": SOURCE_OUTLOOK,
            ...
            "event_count": 0,
            "blocked": True,
            "block_reason": "Ingen hendelser funnet",
            "llm_fallback": True,
            "llm_strategy": {
                "engine": "llm_browser",
                "url": url,
    ...
    def test_llm_fallback_recovers_blocked_source(self, tmp_path):
        """When StrategyDrivenScraper returns events, the source is unblocked."""
        state = PipelineState(tmp_path / "pipeline")
        cfg = _make_config_with_sources([
            {"name": "LLMSource", "type": SOURCE_OUTLOOK, "url": "https://llm.example.com"},
        ])

        blocked_result = self._blocked_llm_source_result("LLMSource")

        with patch(
            "tournament_scheduler.pipeline.stage2_scraping._scrape_source",
            return_value=blocked_result,
        ), patch(
            "tournament_scheduler.pipeline.stage2_scraping.StrategyDrivenScraper",
        ) as mock_scraper_cls:
            mock_instance = MagicMock()
            mock_instance.run.return_value = [_make_event("LLM event")]
        ...
        # Source should be unblocked
        assert result["blocked"] == []
        src = result["sources"][0]
        assert src["blocked"] is False
        assert src["event_count"] == 1
        assert src["llm_fallback"] is True
        assert src.get("llm_fallback_used") is True
        # LLM fallback list in checkpoint should record the recovery
        llm_fb = result["llm_fallback"]
        assert len(llm_fb) == 1
        assert llm_fb[0]["name"] == "LLMSource"
        assert llm_fb[0]["event_count"] == 1
        assert "fallback_reason" in llm_fb[0]

    def test_no_llm_scrape_skips_fallback(self, tmp_path):
        """When no_llm_scrape=True, StrategyDrivenScraper is never called."""
        state = PipelineState(tmp_path / "pipeline")
        cfg = _make_config_with_sources([
            {"name": "LLMSource", "type": SOURCE_OUTLOOK, "url": "https://llm.example.com"},
        ])
        ...
        assert "LLMSource" in result["blocked"]

    def test_llm_fallback_returns_no_events_leaves_source_blocked(self, tmp_path):
        """When StrategyDrivenScraper returns zero events, the source stays blocked."""
        state = PipelineState(tmp_path / "pipeline")
        cfg = _make_config_with_sources([
            {"name": "LLMSource", "type": SOURCE_OUTLOOK, "url": "https://llm.example.com"},
        ])
