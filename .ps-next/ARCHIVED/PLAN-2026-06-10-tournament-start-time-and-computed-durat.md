...
date: 2026-06-10
status: done
feature: "Tournament start time and computed duration/end time"
...
|-----------|---------|-------|
| `tournament_scheduler/season_config.py` contains a `FEDERATION_ROUND_LENGTH_DEFAULTS` dict with an entry for every age group present in `FEDERATION_PARALLEL_GAMES_DEFAULTS` | PASS | All 10 keys (U7,U8,U9,U10,U11,U12,JU10,JU11,JU12,JU13) present with sensible per-age-group minute values |
| The `Tournament` dataclass has a `start_time` field and a duration/end-time helper returning computed HH:MM end time when start_time and round length are provided, None otherwise | PASS | `start_time: Optional[str] = None` field plus `duration_minutes()` and `end_time()` methods implemented in models.py |
| `tournament_scheduler/ical/ical_exporter.py` produces VEVENTs with DTSTART from start_time and DTEND from computed end time, falling back to fixed-hour behavior when start_time unset | PASS | _tournament_start_datetime/_tournament_end_datetime helpers used for per-game, cancelled, and summary events with start_hour fallback |
| Excel overview sheet contains "Starttid" and "Sluttid" columns with non-empty values for tournaments with a start_time | PASS | _OVERVIEW_HEADERS extended; _write_overview_sheet appends start_time and computed end_time |
| Running pytest passes, including new tests for round_length_minutes config validation and Tournament duration/end-time computation | PASS | 287 passed, 2 pre-existing failures in test_stage2_scraping.py unrelated to this change (confirmed pre-existing in plan Log), 9 new tests added |

**Shell checks (ps-verify-plan):** see output below
```
ps-verify-plan: no embedded checks found — exiting 0
```
**Git history:** 7/7 tasks with matching commits
**Tests:** passed (287 passed, 2 pre-existing failures unrelated to this plan, 1 skipped)
