...
|-----------|---------|-------|
| The season planner outputs balanced game counts across all teams within the same age group when multiple teams exist for that age group. | PASS | Deficit-aware seed selection (_deficit_score, _running_game_counts) and deficit-aware cap override implemented in season_planner.py; spread reduced from 17 to 11 in regression test |
| The system produces per-team share warnings that report when game count spreads exceed the configured max_game_count_spread threshold. | PASS | _scan_per_team_share_warnings re-verified unchanged/correct; per_team_share_warnings still flags residual skew per regression test |
| Running pytest tests/test_season_planner.py exits with code 0 and the new real-roster regression test(s) pass, confirming the Jar-vs-Kongsberg U10 spread is reduced compared to the previously documented baseline of 13-18 vs ~25. | PASS | All 53 tests in test_season_planner.py pass; new test asserts spread (11) < 17 baseline and <= 12 new bound |
| The team selection logic in _pick_least_recently_grouped returns teams ordered such that teams furthest below the age-group's expected average game count are not starved relative to over-served teams, even when that requires exceeding _max_club_teams_for for a club. | PASS | _pick_least_recently_grouped sorts by _deficit_score; deficit-aware override allows over-cap candidate selection with scaled club_penalty when its deficit is largest |
| The new club-cap-override counter shows that same-club pairings beyond _max_club_teams_for remain rare (small relative to the total number of tournaments), confirming the cap is bypassed only when needed to reduce a deficit. | PASS | club_cap_overrides property added, surfaced in rules_report and CLI; regression test asserts club_cap_overrides <= total_tournaments (0 of 15 in this run) |
| README.md contains a documented description of the deficit-aware club-cap override behavior. | PASS | README.md new 'Per-club tournament caps and the deficit-aware override' section (line 205+) describes _max_club_teams_for, _deficit_score, deficit-aware override, and club_cap_overrides |

**Shell checks (ps-verify-plan):** see output below
```
ps-verify-plan: no embedded checks found — exiting 0
```
**Git history:** 5 of 5 tasks have matching commits
**Tests:** passed (53 of 53 in tests/test_season_planner.py)
