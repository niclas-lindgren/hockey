...
|-----------|---------|-------|
| Generated season_plan.html and script.js contain HEATMAP_CLUB_COLORS with separate dark/light sets, selected via document.documentElement.dataset.theme | PASS | html_exporter.py emits heatmap_club_colors_json with dark/light keys; script.js HEATMAP_CLUB_COLORS_BY_THEME[currentTheme] selects the active palette for legend and cells |
| [data-theme="light"] defines --heatmap-empty-bg not near-black, and script.js no longer outputs literal rgba(30,41,59,.4) in empty-cell branch | PASS | :root keeps rgba(30,41,59,.4); [data-theme="light"] overrides with rgba(228,228,231,.6); script.js uses var(--heatmap-empty-bg); no literal rgba(30,41,59,.4) remains in templates |
| [data-theme="light"] has --accent-dim darker than --accent (matching dark theme ordering) | PASS | light theme: --accent: #0284c7, --accent-dim: #0369a1 (darker), matching the dark-theme relative ordering |
| .tag--age, .tag--arena, .tag--teams, .tag--travel have light-theme background opacity overrides higher than dark-theme .08 | PASS | [data-theme="light"] overrides set background opacity to .16 (and border to .3) vs base .08/.15 |
| python3 -m py_compile html_exporter.py exits 0 and pytest passes with no new failures from these changes | PASS | py_compile succeeds with no errors; pytest: 320 passed, 1 failed, 2 skipped -- the 1 failure (tests/test_stage2_scraping.py, scraping/threading module) pre-exists at commit 631f87d (before this plan), unrelated to html/css theme changes |
...
**Tests:** passed (320 passed, 1 pre-existing unrelated failure, 2 skipped)
