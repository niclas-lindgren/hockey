...
date: 2026-06-10
status: done
feature: "PLAN: Fix duplicate quality metrics in season plan report (Spredning vs Nye matchups)"
...
|-----------|---------|-------|
| 'Spredning' and 'Nye matchups' show different percentages for varied opponent pairings | PASS | scores.html lines 5/13 differentiate the two metrics with distinct labels and tooltips; end-to-end verification (commit d399c9e) confirmed 1.0 vs 0.267 for a realistic plan |
| _diversity_score() no longer returns _pairwise_matchup_score(), computes opponent variety via _opponent_history | PASS | code review confirms _diversity_score (lines 1209-1260) iterates _opponent_history and computes per-team distinct-opponent ratios, no longer delegating to _pairwise_matchup_score |
| Tests in test_season_planner.py pass and include a case where the two scores diverge | PASS | test_diversity_score_and_pairwise_score_can_diverge and test_diversity_score_returns_zero_when_no_games_scheduled added; suite run shows 4/4 relevant tests pass |
| rich_output.py console summary no longer implies the two metrics are the same | PASS | rich_output.py line 331 reworded to 'Motstandervariasjon (andel mulige motstandere mott)', distinct from pairwise line 'Kampmangfold (andel ferske motstanderpar)' |
| Full suite completes with no failures related to diversity_score, pairwise_matchup_score, or season plan export/report | PASS | 278 passed, 1 skipped, 1 failed (test_zero_events_blocks_source in test_stage2_scraping.py) - failure is unrelated/pre-existing per plan log and confirmed on unmodified tree |
...
**Tests:** passed (278 passed, 1 skipped, 1 pre-existing unrelated failure in stage2 scraping suite)
