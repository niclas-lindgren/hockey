...
|-----------|---------|-------|
| find_available_slots returns hour-level (start, end) time slot tuples per arena fitting required duration | PASS | Implemented in tournament_scheduler/utils/slot_finder.py (find_available_slots), parameterized by required_minutes; covered by tests/test_slot_finder.py (63 tests pass) |
| find_arena_slot_for_date returns fallback club/arena when host arena has no fitting slot, or reports none available | PASS | Implemented in tournament_scheduler/scheduler.py:118 using club_registry.arenas_for_date_search; covered by tests/test_scheduler.py for host-has-slot, fallback, and no-arena-available cases |
| SeasonPlanner.build_plan() produces Tournament.start_time reflecting actual computed free slot, not always 09:00, while passing hosting/game-count warning checks | PASS | season_planner.py wires find_arena_slot_for_date into build_plan, sets start_time and records fallback_host_substitutions; TestSlotAwareScheduling in tests/test_season_planner.py confirms non-default start_time and substitution recording |
| Test run of the three new/updated test files exits 0 with new fallback-host and no-slot-available cases passing | PASS | Ran the three test files: 63 passed, 0 failed |
| Rules/decisions report contains a line naming original host, fallback host, and date for each fallback-host substitution | PASS | season_planner.py rules_report emits 'Vertsbytte <date> (<age_group>)' entries per fallback_host_substitutions entry, naming original and fallback hosts |
...
**Tests:** passed (63/63 in scoped test files; full suite 305 passed, 1 skipped, 2 pre-existing unrelated failures)
