...
date: 2026-06-11
status: done
feature: "Avoid BookUp password leaking to the LLM"
...
|-----------|---------|-------|
| browser_worker.py contains a sanitization function removing value="..." attrs from password/email inputs before _snapshot() returns | PASS | _sanitize_html() defined at line 100, called by _snapshot() at lines 204-205; covered by 6 TestSanitizeHtml tests |
| userMessage() in scraper-agent.ts redacts literal BOOKUP_PASSWORD values from snapshot.html in the resulting message | PASS | redactCredentials() (line 580) applied to snapshot.html/iframe_html/element text inside userMessage() (lines 150,155,161); confirmed by vitest tests |
| llm_scraper.py does not pass BOOKUP_EMAIL/BOOKUP_PASSWORD env var values through to the LLM prompt around capture_dom_snapshot() | PASS | _redact_credential_values() (line 95) applied to visible_text from capture_dom_snapshot() (line 861) and again in _extract_events_via_llm() prompt build (~line 829/869) |
| pytest tests/test_browser_worker.py passes with sanitization tests for password/email input redaction succeeding | PASS | 10 passed (TestSanitizeHtml x6, TestRedactCredentials x4) |
| .pi/lib/scraper-agent.test.ts test suite passes showing redactCredentials() replaces credential substrings with [REDACTED] | PASS | npx vitest run scraper-agent.test.ts: 6 passed |
...
**Tests:** passed (pytest test_browser_worker.py: 10 passed; vitest scraper-agent.test.ts: 6 passed)
