...
- [x] Added tests/test_slot_finder.py (find_available_slots: empty/fully-booked days, single/multiple gaps, cross-date filtering), tests/test_scheduler.py (find_arena_slot_for_date: host-has-slot, host-fully-booked-fallback, no-arena-available, optimal-time scoring), and a new TestSlotAwareScheduling class in tests/test_season_planner.py covering build_plan with events_by_club producing non-default start_time and recorded fallback-host substitutions plus rules_report entries. — 2026-06-10
...
**Commit:** 35b3fb9 (hockey)

### 2026-06-10 — Added tests/test_slot_finder.py (find_available_slots: empty/fully-booked days, single/multiple gaps, cross-date filtering), tests/test_scheduler.py (find_arena_slot_for_date: host-has-slot, host-fully-booked-fallback, no-arena-available, optimal-time scoring), and a new TestSlotAwareScheduling class in tests/test_season_planner.py covering build_plan with events_by_club producing non-default start_time and recorded fallback-host substitutions plus rules_report entries.
**Rationale:** Extended FakeScheduler with find_arena_slot_for_date delegating to a real (I/O-free) TournamentScheduler instance, so SeasonPlanner end-to-end tests exercise the real slot-finding/fallback logic without mocking it away.
**Findings:** pytest tests/test_slot_finder.py tests/test_scheduler.py tests/test_season_planner.py: 63 passed. Full suite: 305 passed (up from 288), 1 skipped, except the same two pre-existing unrelated failures (test_zero_events_blocks_source, test_sources_run_in_different_threads).
LESSONS: arenas_for_date_search iterates ALL CLUB_REGISTRY clubs as fallback candidates, not just roster clubs -- fallback-host tests must mark every known club's calendar as busy except the intended fallback, or an unrelated registry club (e.g. Tonsberg) will win the fallback instead.
**Files:** tests/test_slot_finder.py (+132 new), tests/test_scheduler.py (+131 new), tests/test_season_planner.py (+185/-3)
