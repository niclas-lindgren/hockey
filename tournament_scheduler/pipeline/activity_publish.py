...
_PARTIAL_NUMERIC_DATE_RE = re.compile(r"^(\d{1,2})[./-](\d{1,2})[.]?$")
...
def fetch_pages_branch(*, repo_dir: str = ".", remote: str = "origin", branch: str = "gh-pages") -> None:
    ...
    proc = subprocess.run(["git", "fetch", remote, refspec], cwd=repo_dir, capture_output=True, text=True, timeout=_GIT_TIMEOUT_SECONDS)
    if proc.returncode != 0:
        raise ActivityPublishError(f"Kunne ikke hente {remote}/{branch} før aktivitetskalender-publisering: {proc.stderr.strip()}")


def _resolve_pages_ref(*, repo_dir: str, branch: str) -> str | None:
    for candidate in (branch, f"origin/{branch}", "FETCH_HEAD"):
        proc = subprocess.run(["git", "rev-parse", "--verify", "--quiet", f"{candidate}^{{commit}}"], cwd=repo_dir, capture_output=True, text=True, timeout=_GIT_TIMEOUT_SECONDS)
        if proc.returncode == 0:
            return candidate
    return None


def copy_latest_snapshot(*, repo_dir: str = ".", branch: str = "gh-pages", destination_dir: str | Path) -> int:
    dest = Path(destination_dir)
    dest.mkdir(parents=True, exist_ok=True)
    pages_ref = _resolve_pages_ref(repo_dir=repo_dir, branch=branch)
    if pages_ref is None:
        return 0
    tree_proc = subprocess.run(["git", "ls-tree", "-r", "--name-only", pages_ref, "--", "latest"], cwd=repo_dir, capture_output=True, text=True, timeout=_GIT_TIMEOUT_SECONDS)
    ...
    for branch_path in sorted(line.strip() for line in tree_proc.stdout.splitlines() if line.strip()):
        ...
        if not relative:
            continue
        show_proc = subprocess.run(["git", "show", f"{pages_ref}:{branch_path}"], cwd=repo_dir, capture_output=True, timeout=_GIT_TIMEOUT_SECONDS)
        if show_proc.returncode != 0:
            raise ActivityPublishError(f"Kunne ikke lese {pages_ref}:{branch_path}: {show_proc.stderr.decode('utf-8', errors='replace').strip()}")
...
def _normalise_partial_dates_for_publish(input_path: str | Path) -> Path:
    """Normalize compact ``D.M.`` dates in a temporary copy of SharePoint JSON."""
    ...
    for row in values:
        ...
        for index, value in enumerate(row):
            if not isinstance(value, str):
                continue
            match = _PARTIAL_NUMERIC_DATE_RE.fullmatch(value.strip())
            if match:
    ...
    if not changed:
        return source
    normalised = source.with_name(f"{source.stem}.publish-normalized.json")
    normalised.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return normalised


def prepare_activity_latest_export(*, input_path: str, export_dir: str | Path, repo_dir: str = ".", branch: str = "gh-pages", default_year: int | None = None, generated_at: str | None = None, include_latest_base: bool = True, require_latest_base: bool = True) -> dict[str, Any]:
